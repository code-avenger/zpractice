...
class PrintCapital implements Runnable {
	...
	synchronized (dForPrint) {
	    System.out.println("c index " + dForPrint.index);
	    while (dForPrint.index == 1) {
		System.out.print(dForPrint.capital++);
...
class PrintSmall implements Runnable {
    ...
    public void run() {
	synchronized (dForPrint) {
	    System.out.println("s index " + dForPrint.index);
	    while (dForPrint.index == 2) {
		System.out.print(dForPrint.small++);
		dForPrint.index = 3;
		dForPrint.notifyAll();
	    }
...
class PrintNo implements Runnable {
    ...
    public void run() {
	synchronized (dForPrint) {
	    System.out.println("n index " + dForPrint.index);
